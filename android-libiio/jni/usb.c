...
int usb_context_scan_new(struct iio_scan_result *scan_result, const char *args, int *fd)
{
	fprintf(stderr,"Tracing... usb_context_scan\n");
	...
	// if (ret)
	// 	return ret;
	fprintf(stderr, "parsed vid is %04x, pid is %04x\n", vid, pid);
	libusb_set_option(NULL, LIBUSB_OPTION_NO_DEVICE_DISCOVERY);
	fprintf(stderr, "libusb_set_option succeed\n");
	ret = libusb_init(&ctx);
	if (ret < 0) {
		fprintf(stderr, "initialize failed\n");
		return -(int) libusb_to_errno(ret);
	}
	fprintf(stderr, "libusb init succeed\n");
	libusb_wrap_sys_device(ctx, (intptr_t) *fd, &hdl);
	fprintf(stderr, "libusb_wrap_sys_device succeed\n");
	dev = libusb_get_device(hdl);
	fprintf(stderr, "After libusb_get_device\n");
	unsigned int intrfc = 0;
	struct libusb_device_descriptor device_descriptor;
	if(vid || pid) {
		fprintf(stderr, "Before libusb_get_device_descriptor\n");
		ret = libusb_get_device_descriptor(dev, &device_descriptor);
		if (ret)
		{
			fprintf(stderr,"ret failed with %d\n", ret);
			goto cleanup_libusb_exit;
		}
		if ((vid && vid != device_descriptor.idVendor)|| (pid && pid != device_descriptor.idProduct))
		{
			fprintf(stderr,"vid or pid not match\n");
			goto cleanup_libusb_exit;
		}
		fprintf(stderr, "Get right device\n");
		// if (pid && pid != device_descriptor.idProduct)
	}
	fprintf(stderr,"Before open libusb_open\n");
	// ret = libusb_open(dev, &hdl);
	// if (ret)
	// {
	// 	fprintf(stderr,"open libusb failed\n");
	// 	goto cleanup_libusb_exit;
	// }
...
int usb_context_scan(struct iio_scan_result *scan_result, const char *args)
{
	...
	int fd;
	struct libusb_device_handle *hdl;
	char *usbdir = "/dev/bus/usb/001/002";

	sscanf(usbdir, "%d", &fd);
	fprintf(stderr,"usbdir is %s, fd is %d\n", usbdir, fd);
	ret = parse_vid_pid(args, &vid, &pid);
	vid=0x0456;
	pid=0xb673;
	if (ret)
