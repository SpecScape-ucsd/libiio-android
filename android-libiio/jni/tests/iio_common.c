...
struct iio_context * autodetect_context(bool rtn, const char * name, const char * scan, int *fd)
{
	fprintf(stderr,"Tracing autodetect_context\n");
	...
	fprintf(stderr,"Create scan context successfully\n");
	fprintf(stderr, "autodetect_context fd is %d\n", *fd);
	ret = iio_scan_context_get_info_list_new(scan_ctx, &info, fd);
	if (ret < 0) {
		...
		const struct option *options, const char *options_descriptions[], int *fd)
{
	...
	while ((c = getopt_long(argc, argv, buf,	/* Flawfinder: ignore */
		...
		case 'S':
			...
			if (optarg) {
				arg = optarg;
				fprintf(stderr,"input optarg is %s\n", optarg);
			} else {
				if (argc > optind && argv[optind] && argv[optind][0] != '-')
					arg = argv[optind++];
			}
	...
	if (do_scan) {
		fprintf(stderr, "handle_common_opts fd is %d\n", *fd);
		autodetect_context(false, name, arg, fd);
		return NULL;
	} else if (detect_context || backend == IIO_AUTO)
		ctx = autodetect_context(true, name, arg, fd);
