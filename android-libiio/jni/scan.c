...
ssize_t iio_scan_context_get_info_list_new(struct iio_scan_context *ctx,
		struct iio_context_info ***info, int *fd)
{
	...
	size_t i, j = 0;
	ssize_t ret;
	fprintf(stderr,"Tracing... iio_scan_context_get_info_list\n");
	for (token = iio_strtok_r(ctx->backendopts, ",", &rest);
		...
		if (WITH_LOCAL_BACKEND && !strcmp(token, "local")) {
			...
			fprintf(stderr,"iio_scan_context_get_info_list_new fd is %d\n", *fd);
			ret = usb_context_scan_new(&scan_result, token, fd);
