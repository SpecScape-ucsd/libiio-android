...
ssize_t iio_scan_context_get_info_list_android(struct iio_scan_context *ctx,
		struct iio_context_info ***info, int *fd)
{
	fprintf(stderr,"Tracking... iio_scan_context_get_info_list_android\n");
	...
	for (token = iio_strtok_r(ctx->backendopts, ",", &rest);
		...
		if (WITH_LOCAL_BACKEND && !strcmp(token, "local")) {
			...
			ret = usb_context_scan_android(&scan_result, token, fd);
