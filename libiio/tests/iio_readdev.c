...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argw, "+" COMMON_OPTIONS MY_OPTS,	/* Flawfinder: ignore */
		...
		case 'b':
			...
			// fprintf(stderr, "buffer size is %s\n", optarg);
			break;
		case 'B':
			benchmark = true;
			break;
		case 's':
			...
			// fprintf(stderr, "sample size is %s\n", optarg);
			break;
		case '?':
	...
	// fprintf(stderr, "Before iio_context_find_device\n");
	...
	// fprintf(stderr, "iio_context_find_device successfully\n");
	if (trigger_name) {
