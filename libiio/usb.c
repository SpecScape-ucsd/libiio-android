...
static struct iio_context * usb_create_context_android(unsigned int bus,
					       uint16_t address, uint16_t intrfc, int *fd)
{
	libusb_context *usb_ctx;
	libusb_device_handle *hdl = NULL;
	const struct libusb_interface_descriptor *iface;
	...
	device = libusb_get_device(hdl);
	if (bus == libusb_get_bus_number(device) &&
	...
	// fprintf(stderr,"usb_create_context_android: Get usb device successfully\n");
#if defined(LIBUSB_API_VERSION) && (LIBUSB_API_VERSION >= 0x01000016)
	libusb_set_auto_detach_kernel_driver(hdl, true);
#endif
	...
	// fprintf(stderr, "usb_create_context_android: Interface claimed successfully\n");
	ret = libusb_get_active_config_descriptor(usb_dev, &conf_desc);
	if (ret) {
		ret = -(int) libusb_to_errno(ret);
		iio_strerror(-ret, err_str, sizeof(err_str));
	...
	// fprintf(stderr, "usb_create_context_android: Valid configuration of endpoints\n");
	...
	if (ret)
		goto err_free_endpoints;
	// fprintf(stderr, "usb_create_context_android: usb_io_context_init successfully\n");
	/* We reserve the first I/O endpoint couple for global operations */
	pdata->io_ctx.ep = &pdata->io_endpoints[0];
	pdata->io_ctx.ep->in_use = true;
	...
	// fprintf(stderr, "usb_create_context_android: usb_reset_pipes successfully\n");
	...
	// fprintf(stderr, "usb_create_context_android: usb_open_pipe successfully\n");
	ctx = iiod_client_create_context(pdata->iiod_client, &pdata->io_ctx);
	if (!ctx) {
		ret = -errno;
		goto err_reset_pipes;
	}
	// fprintf(stderr, "usb_create_context_android: iiod_client_create_context successfully\n");
	libusb_free_config_descriptor(conf_desc);
	// fprintf(stderr, "usb_create_context_android: libusb_free_config_descriptor successfully\n");
	ctx->name = "usb";
	ctx->ops = &usb_ops;
	ctx->pdata = pdata;

	for (i = 0; i < iio_context_get_devices_count(ctx); i++) {
		// fprintf(stderr, "usb_create_context_android: iio_context_get_devices_count successfully\n");
		struct iio_device *dev = iio_context_get_device(ctx, i);
	...
	if (ret < 0)
		goto err_context_destroy;
	// fprintf(stderr, "usb_create_context_android: usb_populate_context_attrs successfully\n");
	return ctx;
...
struct iio_context * usb_create_context_from_uri_android(const char *uri, int *fd)
{
	long bus, address, intrfc;
	char *end;
	const char *ptr;
	...
	if (!*ptr) {
		...
		scan_ctx = iio_create_scan_context("usb", 0);
		if (!scan_ctx) {
			fprintf(stderr, "iio_create_scan_context failed\n");
			errno = ENOMEM;
			goto err_bad_uri;
		}
		ret = iio_scan_context_get_info_list_android(scan_ctx, &info, fd);
		if (ret < 0) {
			iio_scan_context_destroy(scan_ctx);
			errno = ENOMEM;
			goto err_bad_uri;
	...
	return usb_create_context_android((unsigned int) bus,
			(uint16_t) address, (uint16_t) intrfc, fd);
...
int usb_context_scan_android(struct iio_scan_result *scan_result, const char *args, int *fd)
{
	fprintf(stderr,"Tracking... usb_context_scan_android\n");
	...
	libusb_wrap_sys_device(ctx, (intptr_t) *fd, &hdl);
