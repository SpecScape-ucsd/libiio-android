...
void getvidpid(char *usbarginput, int argc, char **argv){
    int ret;
    char vid[5];
    char pid[5];
...
int main(int argc, char **argv) {
    // char *usbarginput = "usb";
    char usbarginput[14];
    getvidpid(usbarginput, argc, argv);
    fprintf(stderr,"usbarginput is %s\n", usbarginput);

    int newargc = 3;
    char *newargv[3];
    fprintf(stderr,"MY_NAME length is %d\n", strlen(MY_NAME));
    ...
    ctx = handle_common_opts(MY_NAME, newargc, argw, MY_OPTS, options, options_descriptions);
