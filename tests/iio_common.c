...
struct iio_context * autodetect_context(bool rtn, const char * name, const char * scan, libusb_device *device)
{
	...
	ret = iio_scan_context_get_info_list_new(scan_ctx, &info, device);
	if (ret < 0) {
...
struct iio_context * handle_common_opts(char * name, int argc,
		...
		const struct option *options, const char *options_descriptions[], libusb_device *device)
{
	struct iio_context *ctx = NULL;
	int c;
	...
	if (do_scan) {
		autodetect_context(false, name, arg, device);
		return NULL;
	} else if (detect_context || backend == IIO_AUTO)
		ctx = autodetect_context(true, name, arg, device);
