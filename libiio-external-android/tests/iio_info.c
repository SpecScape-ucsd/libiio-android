...
int main(int argc, char **argv)
{
	...
	argw = dup_argv(MY_NAME, argc-1, argv);
	...
	ctx = handle_common_opts_android(MY_NAME, argc-1, argw, MY_OPTS, options, options_descriptions, &fd);
	// fprintf(stderr, "handle_common_opts_android finsihed, argc is %d, argw is %s\n", argc, argw[1]);
	opts = add_common_options(options);
	if (!opts) {
		fprintf(stderr, "Failed to add common options\n");
		return EXIT_FAILURE;
	}
	while ((c = getopt_long(argc-1, argw, "+" COMMON_OPTIONS MY_OPTS "s", /* Flawfinder: ignore */
					opts, NULL)) != -1) {
		switch (c) {
		...
		case 'a':
			...
			break;
		case 's':
			autodetect_context_android(false, MY_NAME, NULL, &fd);
			return EXIT_SUCCESS;
		case '?':
