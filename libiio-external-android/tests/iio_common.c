...
struct iio_context * autodetect_context_android(bool rtn, const char * name, const char * scan, int *fd)
{
	// fprintf(stderr,"Tracking... autodetect_context_android\n");
...
struct iio_context * handle_common_opts_android(char * name, int argc,
		char * const argv[], const char *optstring,
		const struct option *options, const char *options_descriptions[], int *fd)
{
	struct iio_context *ctx = NULL;
	int c;
	...
	if (do_scan) {
		autodetect_context_android(false, name, arg, fd);
		// fprintf(stderr,"autodetect_context_android finished\n");
		return NULL;
	} else if (detect_context || backend == IIO_AUTO)
		ctx = autodetect_context_android(true, name, arg, fd);
		...
		ctx = iio_create_context_from_uri_android(arg, fd);
	else
		ctx = iio_create_default_context();
	// fprintf(stderr, "handle_common_opts_android: Successfully get back to iio_common\n");
	...
	// fprintf(stderr, "handle_common_opts_android: Able to find IIO context\n");
	if (ctx && timeout >= 0) {
	...
	// fprintf(stderr,"handle_common_opts_android: set iio_context timeout successfully\n");
	return ctx;
}
